...
public extension UIView {
    ...
    /// this from the owner's layout pass — `viewDidLayoutSubviews()` in a view controller, or
    /// `layoutSubviews()` in a `UIView` subclass — for a shadowed view that changes size, e.g.
    /// a control whose title grows with Dynamic Type. Views that keep their size never need it.
